...
Public Class PALFunctions
    ...
    Public Function ExportThresholdFileToPerfmonTemplate(ByVal sThresholdFilePath As String) As String
        ...
        '                If sExistingCounterObject.IndexOf("SQLServer:") > 0 Then
        ...
        Next
        'sOutput = sOutput & "	<PARAM NAME=" & Chr(34) & "Counter00001.Path" & Chr(34) & " VALUE=" & Chr(34) & "\LogicalDisk(*)\*" & Chr(34) & "/>" & vbNewLine
    ...
    Public Function ExportThresholdFileToDataCollectorTemplate(ByVal sThresholdFilePath As String) As String
        ...
        sOutput = sOutput & _
        "</PerformanceCounterDataCollector>" & vbNewLine & _
        "<DataManager>" & vbNewLine & _
        "    <Enabled>-1</Enabled>" & vbNewLine & _
        "    <CheckBeforeRunning>-1</CheckBeforeRunning>" & vbNewLine & _
        "    <MinFreeDisk>200</MinFreeDisk>" & vbNewLine & _
        "    <MaxSize>1024</MaxSize>" & vbNewLine & _
        "    <MaxFolderCount>100</MaxFolderCount>" & vbNewLine & _
        "    <ResourcePolicy>0</ResourcePolicy>" & vbNewLine & _
        "    <FolderAction>" & vbNewLine & _
        "        <Size>0</Size>" & vbNewLine & _
        "        <Age>1</Age>" & vbNewLine & _
        "        <Actions>3</Actions>" & vbNewLine & _
        "    </FolderAction>" & vbNewLine & _
        "    <FolderAction>" & vbNewLine & _
        "        <Size>0</Size>" & vbNewLine & _
        "        <Age>56</Age>" & vbNewLine & _
        "        <Actions>8</Actions>" & vbNewLine & _
        "    </FolderAction>" & vbNewLine & _
        "    <FolderAction>" & vbNewLine & _
        "        <Size>0</Size>" & vbNewLine & _
        "        <Age>168</Age>" & vbNewLine & _
        "        <Actions>26</Actions>" & vbNewLine & _
        "    </FolderAction>" & vbNewLine & _
        "    <ReportSchema>" & vbNewLine & _
        "        <Report name=" & Chr(34) & "PAL Report" & Chr(34) & " version=" & Chr(34) & "1" & Chr(34) & " threshold=" & Chr(34) & "100" & Chr(34) & ">" & vbNewLine & _
        "            <Import file=" & Chr(34) & "%systemroot%\pla\reports\Report.System.Common.xml" & Chr(34) & "/>" & vbNewLine & _
        "            <Import file=" & Chr(34) & "%systemroot%\pla\reports\Report.System.Summary.xml" & Chr(34) & "/>" & vbNewLine & _
        "            <Import file=" & Chr(34) & "%systemroot%\pla\reports\Report.System.Performance.xml" & Chr(34) & "/>" & vbNewLine & _
        "            <Import file=" & Chr(34) & "%systemroot%\pla\reports\Report.System.CPU.xml" & Chr(34) & "/>" & vbNewLine & _
        "            <Import file=" & Chr(34) & "%systemroot%\pla\reports\Report.System.Network.xml" & Chr(34) & "/>" & vbNewLine & _
        "            <Import file=" & Chr(34) & "%systemroot%\pla\reports\Report.System.Disk.xml" & Chr(34) & "/>" & vbNewLine & _
        "            <Import file=" & Chr(34) & "%systemroot%\pla\reports\Report.System.Memory.xml" & Chr(34) & "/>" & vbNewLine & _
        "        </Report>" & vbNewLine & _
        "    </ReportSchema>" & vbNewLine & _
        "    <Rules>" & vbNewLine & _
        "        <Logging level=" & Chr(34) & "15" & Chr(34) & " file=" & Chr(34) & "rules.log" & Chr(34) & "/>" & vbNewLine & _
        "        <Import file=" & Chr(34) & "%systemroot%\pla\rules\Rules.System.Common.xml" & Chr(34) & "/>" & vbNewLine & _
        "        <Import file=" & Chr(34) & "%systemroot%\pla\rules\Rules.System.Summary.xml" & Chr(34) & "/>" & vbNewLine & _
        "        <Import file=" & Chr(34) & "%systemroot%\pla\rules\Rules.System.Performance.xml" & Chr(34) & "/>" & vbNewLine & _
        "        <Import file=" & Chr(34) & "%systemroot%\pla\rules\Rules.System.CPU.xml" & Chr(34) & "/>" & vbNewLine & _
        "        <Import file=" & Chr(34) & "%systemroot%\pla\rules\Rules.System.Network.xml" & Chr(34) & "/>" & vbNewLine & _
        "        <Import file=" & Chr(34) & "%systemroot%\pla\rules\Rules.System.Disk.xml" & Chr(34) & "/>" & vbNewLine & _
        "        <Import file=" & Chr(34) & "%systemroot%\pla\rules\Rules.System.Memory.xml" & Chr(34) & "/>" & vbNewLine & _
        "    </Rules>" & vbNewLine & _
        "</DataManager>" & vbNewLine & _
        "</DataCollectorSet>"
...
Public Class PALCommandObject
    ...
    Public dctQuestionCollection As New Dictionary(Of String, QuestionObject)
    ...
    Public Function BuildCommandLine() As String
        ...
        Dim sBatchLine, sLogFile, sThresholdFile, sInterval, sOutputDirectory, sIsOutputHTML, sIsOutputXML, sHTMLOutputFileName, sXMLOutputFileName, sAllCounterStats, sBeginTime, sEndTime, sNumberOfThreads As String

        sLogFile = "-Log " & AddSingleQuotesOrNot(CounterLogPaths)
        sThresholdFile = "-ThresholdFile " & AddSingleQuotesOrNot(ThresholdFilePath)
        sInterval = "-Interval " & AddSingleQuotesOrNot(ConvertIntervalToSeconds(AnalysisInterval))
        ...
        sHTMLOutputFileName = "-HtmlOutputFileName " & AddSingleQuotesOrNot(HTMLOutputFileName)
        sXMLOutputFileName = "-XmlOutputFileName " & AddSingleQuotesOrNot(XMLOutputFileName)

        If TimeRangeRestriction = True Then
            sBeginTime = "-BeginTime " & Chr(39) & BeginTime.ToString & Chr(39)
            sEndTime = "-EndTime " & Chr(39) & EndTime.ToString & Chr(39)
        Else
            sBeginTime = ""
            sEndTime = ""
        End If
        ...
        If OutputDirectory = DEFAULT_OUTPUT_DIRECTORY Then
            ...
            sOutputDirectory = ""
        Else
            sOutputDirectory = "-OutputDir " & AddSingleQuotesOrNot(OutputDirectory)
        End If
        sNumberOfThreads = "-NumberOfThreads " & NumberOfThreads
        If sOutputDirectory = "" Then
            sBatchLine = "Powershell -NoProfile " & Chr(34) & ".\PAL.ps1" & Chr(34) & " " & sLogFile & " " & sThresholdFile & " " & sInterval & " " & sIsOutputHTML & " " & sHTMLOutputFileName & " " & sIsOutputXML & " " & sXMLOutputFileName & " " & sAllCounterStats & " " & sNumberOfThreads
        Else
            sBatchLine = "Powershell -NoProfile " & Chr(34) & ".\PAL.ps1" & Chr(34) & " " & sLogFile & " " & sThresholdFile & " " & sInterval & " " & sIsOutputHTML & " " & sHTMLOutputFileName & " " & sIsOutputXML & " " & sXMLOutputFileName & " " & sAllCounterStats & " " & sOutputDirectory & " " & sNumberOfThreads
        End If
        ...
        If dctQuestionCollection.Count > 0 Then
            For Each sKey In dctQuestionCollection.Keys
                If dctQuestionCollection(sKey).QuestionDataType = "boolean" Then
                    ...
                    End If
                Else
                    sBatchLine = sBatchLine & " " & "-" & sKey & " '" & dctQuestionCollection(sKey).sAnswer & "'"
                End If
            Next
        End If
        If TimeRangeRestriction = True Then
    ...
    Public Function BuildQueryStyleCommandLine() As String
        ...
        sNumberOfThreads = "-NumberOfThreads " & NumberOfThreads
        If sOutputDirectory = "" Then
            sBatchLine = "Powershell " & Chr(34) & ".\PAL.ps1" & Chr(34) & vbNewLine & sLogFile & vbNewLine & sThresholdFile & vbNewLine & sInterval & vbNewLine & sIsOutputHTML & vbNewLine & sHTMLOutputFileName & vbNewLine & sIsOutputXML & vbNewLine & sXMLOutputFileName & vbNewLine & sAllCounterstats & vbNewLine & sNumberOfThreads
        Else
            sBatchLine = "Powershell " & Chr(34) & ".\PAL.ps1" & Chr(34) & vbNewLine & sLogFile & vbNewLine & sThresholdFile & vbNewLine & sInterval & vbNewLine & sIsOutputHTML & vbNewLine & sHTMLOutputFileName & vbNewLine & sIsOutputXML & vbNewLine & sXMLOutputFileName & vbNewLine & sOutputDirectory & vbNewLine & sAllCounterstats & vbNewLine & sNumberOfThreads
        End If
        If LCase(IsOutputHTML) = "$true" Then
...
Public Class PALCommandQueueObject
    ...
    Sub SaveTextAsBatchFile(ByVal sFilePath As String, ByVal sText As String, ByVal sPalInstallationDirectory As String)
        ...
        sText = sPalInstallDriveLetter & vbNewLine & sText
